...
class OpenIdMixin(object):
    ...
    def _openid_args(self, callback_uri, ax_attrs=[], oauth_scope=None):
        ...
        if oauth_scope:
            args.update({
                ...
                "openid.oauth.consumer": self.request.host, #"http://contacts.mozillalabs.com", # self.request.host.split(":")[0],
    ...
    def _on_authentication_verified(self, callback, response):
        ...
        for name, values in self.request.arguments.iteritems():
            logging.error("name: %s; values: %s" % (name,values))

        claimed_id = None
        if "openid.claimed_id" in self.request.arguments:
          claimed_id = self.request.arguments["openid.claimed_id"][-1]
...
class OAuthMixin(object):
    ...
    def authorize_redirect(self, callback_uri=None, extra_params=None):
        ...
        if getattr(self, "_OAUTH_VERSION", "1.0a") == "1.0a":
            ...
            http.fetch(self._oauth_request_token_url(callback_uri=callback_uri), #MRH added callback_uri... 
                self.async_callback(
                  self._on_request_token, 
                  self._OAUTH_AUTHORIZE_URL, 
                  callback_uri))
